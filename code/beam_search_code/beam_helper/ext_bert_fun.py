...
'''
<EXTERNAL CODE>
Credits: https://github.com/todd-cook/ML-You-Can-Use
'''
...
if device == "cpu":
    print("Device fetched for perplexity calculation is CPU. Abandoning job.")
    assert (device != "cpu")
...
('.', ('.',), (0.8961634635925293,)))
    ...
    if total_tokens > 510:
        LOG.warning(
            "Too many tokens, should be 510 or less, found %s",
            total_tokens)
        return None
    LOG.debug(
        "# bert tokens: %s # whole tokens: %s",
        total_tokens,
        len(whole_tokens))
    ...
    for idx in bert_token_map:
        ...
        with torch.no_grad():
            ...
            indexed_tokens = list(start_token_id) + \
                the_tokens + list(end_token_id)
            LOG.debug("index tokens %s", ",".join(
                [str(tmp) for tmp in indexed_tokens]))
            ...
            predictions = softmax(outputs[0].squeeze(0)).detach().cpu()
...
('everything', 0.03310466185212135), ('this', 0.028820807114243507))
    ...
    # to find the true index of the desired word; count all of the tokens and
    # subtokens before
    the_word_idx = (
        len(
            list(
                chain.from_iterable(
                    [vals for key, vals in bert_token_map.items() if key <
                     word_index]
                )
            )
        )
...
def get_word_in_sentence_probability(
    ...
    if len(tokens_to_predict) == 1:
        return tuple([predictions[starting_position]
                     [tokens_to_predict].item()])  # type: ignore
    return tuple(
        [
            (
...
def sum_log_probabilities(
    ...
    return sum([math.log(tmp)
               for tmp in chain.from_iterable(data)])  # type: ignore
